...
export function VersionRail({
  ...
  const [previewPosition, setPreviewPosition] = React.useState<number | null>(
    null,
  );
  ...
  const activityData = React.useMemo(() => {
    ...
    localVersions.forEach((version, index) => {
      ...
      const prevContent =
        index === 0
          ...
          : localVersions[index - 1]?.content || '';
      const activity = Math.min(
        50,
        Math.max(5, (version.content || '').length - prevContent.length),
      );
    ...
    if (currentContent.trim()) {
      const prevContent =
        localVersions[localVersions.length - 1]?.content ||
        versions[versions.length - 1]?.content ||
        '';
      const activity = Math.min(
        50,
        Math.max(5, currentContent.length - prevContent.length),
      );
  ...
  React.useEffect(() => {
    ...
    window.addEventListener(
      ...
      handleEditorChange as EventListener,
    );
    window.addEventListener(
      'editor:content-changed',
          ...
          window.dispatchEvent(
            new CustomEvent('preview-document-update', {
              detail: {
                ...
                newContent: dataPoint.version.content,
              },
            }),
          );
        }
  ...
  const confirmDestructiveChange = React.useCallback(() => {
    if (previewPosition !== null) {
      ...
      if (dataPoint?.version) {
        ...
        window.dispatchEvent(
          new CustomEvent('editor:force-content-update', {
  ...
  const getPositionFromMousePosition = (
    ...
    return Math.min(
      Math.max(Math.round(fraction * (activityData.length - 1)), 0),
  ...
  return (
    <div className="w-full">
      ...
      <div
        ...
        role="slider"
        aria-label="Document timeline scrubber"
        aria-valuenow={currentIndex}
        aria-valuemin={0}
        aria-valuemax={Math.max(activityData.length - 1, 0)}
        ...
        {isExpanded && (
          <div className="absolute inset-0">
            {activityData.map((dataPoint, index) => {
              ...
              return (
                <div
                  key={dataPoint.version?.id || index}
                ...
                >
                  ...
                  {isHovered && dataPoint.version && (
                    <div
                      className={`absolute bg-background border px-2 py-1 text-xs rounded shadow-md z-20 whitespace-nowrap ${
                        index === 0
                          ? 'left-0 -bottom-10'
                    ...
                    >
                      <div className="text-xs">
                        {formatDistanceToNow(dataPoint.timestamp, {
                          addSuffix: true,
                        })}
                        {isCurrent && (
                          <span className="ml-1 text-green-500">(now)</span>
                        )}
                      </div>
                    </div>
                  )}
