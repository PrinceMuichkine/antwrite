...
export function useDocument() {
      ...
      mutate(
        (prev) =>
          typeof update === 'function'
            ? (update as any)(prev ?? initialDocument)
            : update,
        false,
      );
    },
  ...
  const createNewDocument = useCallback(async () => {
    ...
    try {
      ...
      if (!docResponse.ok) {
        const docErrorData = await docResponse
          .json()
          .catch(() => ({ error: 'Unknown error' }));
        console.error(
          '[Document] Failed to create document:',
          docResponse.status,
          docErrorData,
        );
        throw new Error(
          `Failed to create document: ${docResponse.status} ${docErrorData.error || ''}`,
        );
      }
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response
            .json()
            .catch(() => ({ error: 'Unknown error during rename' }));
          throw new Error(
            `Failed to rename document: ${errorData.error || response.statusText}`,
          );
        }
        ...
        if (typeof window !== 'undefined') {
          window.dispatchEvent(
            new CustomEvent('document-renamed', {
              detail: {
  ...
  const loadDocument = useCallback(
    async (documentId: string, params?: LoadDocumentParams) => {
      try {
  ...
  const deleteDocument = useCallback(
    async (documentId: string, params?: DeleteDocumentParams) => {
      try {
        if (
          !documentId ||
          ...
          console.error(
            '[useDocument] Invalid document ID for deletion:',
            documentId,
          );
